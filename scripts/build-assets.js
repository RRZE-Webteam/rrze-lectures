...
function ensureString(value, label) {
    if (typeof value !== 'string' || value.trim() === '') {
        throw new Error('Ungueltiger Eintrag in package.json: ' + label);
    }
...
function getBuildPaths(projectRoot) {
    ...
    if (!pkg.source || typeof pkg.source !== 'object') {
        throw new Error('package.json enthaelt keinen gueltigen "source"-Block');
    }

    if (!pkg.target || typeof pkg.target !== 'object') {
        throw new Error('package.json enthaelt keinen gueltigen "target"-Block');
    }
...
function main() {
    ...
    if (args.watch) {
        return runWatch('dev', buildPaths, projectRoot).then(function onWatchStarted() {
            console.log('Erledigt. Watch-Modus laeuft.');
        });
    }

    return runOnce(args.mode, buildPaths)
        .then(function onBuildDone(result) {
            logBuiltFiles('JS gebaut', result.js, projectRoot);
            logBuiltFiles('CSS gebaut', result.css, projectRoot);

            if (args.mode === 'prod') {
                logBuiltFiles('Source-Maps entfernt', result.removedSourceMaps.js.concat(result.removedSourceMaps.css), projectRoot);
            }
        })
        .catch(function onBuildError(err) {
            console.error(err);
            process.exit(1);
        });
}
