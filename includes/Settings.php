...
class Settings {
    ...
    public function __construct($pluginFile)
    {
        ...
        $oldOptions = get_option('_rrze_lecture');
        $updated = get_option('lecture-updated');
        if (!empty($oldOptions) && empty($updated)) {
            foreach ($oldOptions as $k => $v) {
                $oldOptions['basic_' . $k] = $v;
            }
            if (empty($oldOptions['basic_lecture_url'])) {
                $oldOptions['basic_lecture_url'] = 'https://lecture.uni-erlangen.de';
            }
            update_option('rrze-lectures', $oldOptions);
            update_option('lecture-updated', 1);
        }
    }
            ...
            <form method="post" id="search-fauorgnr-form">
                <table class="form-table" role="presentation" class="striped">
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <input type="button" id="search-fauorgnr-button" class="button button-primary"
                                    value="<?php echo __('Search', 'rrze-lectures'); ?>">
                            </td>
                            <td>
                                <div id="search-fauorgnr-loading"><i class="fa fa-refresh fa-spin fa-2x aligncenter"></i></div>
                            </td>
                        </tr>
                    </tbody>
                </table>
